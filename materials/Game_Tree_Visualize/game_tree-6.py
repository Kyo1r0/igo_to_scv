...
def main():
    ...
    # --- 変更点: 黒先手と白先手で完全に処理を分離 ---

    # --- 黒先手の場合の処理 ---
    print("--- 黒先手の分析を開始 ---")
    dot_black = graphviz.Digraph(comment='Black to Play First')
    dot_black.attr(bgcolor='lightgray', rankdir='TB', label='Black to Play First', fontsize='20')
    ...
    try:
        # view=Falseにして、自動で画像を開かないようにする
        dot_black.render('black_first', format='png', view=False, cleanup=True)
        print("ゲーム木を black_first.png として保存しました。")
    except Exception as e:
        print(f"エラー: black_first.png の保存中に問題が発生しました: {e}")

    # --- 白先手の場合の処理 ---
    print("\n--- 白先手の分析を開始 ---")
    dot_white = graphviz.Digraph(comment='White to Play First')
    dot_white.attr(bgcolor='lightgray', rankdir='TB', label='White to Play First', fontsize='20')
